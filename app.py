...
def render_summary_metrics(schedules_df: pd.DataFrame, today_events: pd.DataFrame, month_events: pd.DataFrame) -> None:
...
def render_today_view(today_events: pd.DataFrame) -> None:
    st.subheader("오늘 확인해야 할 목록")
    render_event_cards(today_events, "오늘 날짜와 일치하는 일정이 아직 없습니다.")


def render_upcoming_view(upcoming_events: pd.DataFrame) -> None:
    st.subheader("가까운 일정")
    render_event_cards(upcoming_events, "앞으로 7일 안에 표시할 일정이 없습니다.")


def render_month_view(month_events: pd.DataFrame) -> None:
...
def render_all_schedule_view(schedules_df: pd.DataFrame, events_df: pd.DataFrame) -> None:
...
def render_dashboard() -> None:
    ...
    render_summary_metrics(schedules_df, today_events, month_events)

    write_tab, today_tab, upcoming_tab, month_tab, all_tab = st.tabs(
        ["작성", "오늘 확인", "가까운 일정", "이번 달", "전체 일정표"]
    )
    with write_tab:
        render_input_form()
        render_file_tools()
    with today_tab:
        render_today_view(today_events)
    with upcoming_tab:
        render_upcoming_view(upcoming_events)
    with month_tab:
        render_month_view(month_events)
    with all_tab:
        render_all_schedule_view(schedules_df, events_df)
